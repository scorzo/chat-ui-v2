...
from .career_goals_coaching_models import CareerGoalsCoaching
import os
...
class EditCareerGoalsCoachingParams(BaseModel):
...
class EditCareerGoalsCoachingTool(BaseTool):
    ...
    args_schema: Type[BaseModel] = EditCareerGoalsCoachingParams
    ...
    def edit_datanode(self, prompt: str, node_id: str) -> dict:
        ...
        Respond with the data as a JSON object based on the CareerGoalsCoaching Pydantic model format.
        ...
        result = edit_datanode_from_model_with_tools(
            prompt=prompt,
            model_name="gpt-4o",
            pydantic_model=CareerGoalsCoaching,
            #tools=[],  # Add appropriate tools if needed
            node_id=node_id,
            node_type="CareerGoalsCoachingModalContent",
...
if __name__ == "__main__":
    tool = EditCareerGoalsCoachingTool()
    result = tool._run(prompt="Update the career goals with the latest achievements and resources", node_id="target_node_id")
