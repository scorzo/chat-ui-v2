...
class FreeBusyTool(BaseTool):
    ...
    async def _arun(
        ...
        return await asyncio.to_thread(self.check_free_busy, calendar_id, start_time, end_time, timezone)
    ...
    def check_free_busy(self, calendar_id: str, start_time: Optional[str], end_time: Optional[str], timezone: str) -> str:
        ...
        try:
            ...
            if not busy_times:
                return json.dumps({'message': 'No busy times found in that time span.'})
            ...
            while current_time < end_time_dt:
                ...
                if free_slot_end > free_slot_start:
                    free_times.append({"start": free_slot_start.isoformat(), "end": free_slot_end.isoformat()})
                    print(f"From {free_slot_start} to {free_slot_end}")

                current_time = free_slot_end

            if not free_times:
                return json.dumps({"message": "No free times found."})

            return json.dumps({"free_times": free_times})
